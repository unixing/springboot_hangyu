...
public class TravellerTicketInfoExcelExample extends AbstractJExcelView{
	...
	public TravellerTicketInfoExcelExample(Map<String, Object> map) {
		this.map = map;
	}
	...
	public void buildExcelDocument(Map<String, Object> dataMap,WritableWorkbook work, HttpServletRequest req,HttpServletResponse response) {
		...
		try {
			...
			List<TicketInfo> list = (List<TicketInfo>) dataMap.get("list");
	...
	private void writeContext(WritableSheet wsheet, List<TicketInfo> list) {
		//拿到数据长度
		int rows = list.size();
		//jsl对象
