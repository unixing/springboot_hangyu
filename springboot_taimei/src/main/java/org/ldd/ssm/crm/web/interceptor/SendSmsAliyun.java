...
//                SingleSendSmsResponse httpResponse = client.getAcsResponse(request);
    ...
    public static boolean sendPriceWatching(WatchRemindDto dto) {     
        ...
        try {
            ...
            request.setRecNum(dto.getUserTel());//接收手机号，多个用英文逗号隔开
        	//验证码生成
            request.setParamString("{legs:\""+dto.getLegs()+"\",num:\""+dto.getNum()+"\",leg:\""+dto.getLeg()+"\",changeRate:\""+dto.getChangeRate()+"\",changePrice:\""+dto.getChangePrice()+"\"}");//模板中的参数
            SingleSendSmsResponse response = client.getAcsResponse(request);
        } catch (ServerException e) {
            e.printStackTrace();
            result = false;
