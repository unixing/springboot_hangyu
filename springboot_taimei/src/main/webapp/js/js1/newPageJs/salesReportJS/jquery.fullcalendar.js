...
(function ($) {
	...
	function init(target) {
        ...
        '<div class="time-set-year calendar-year"></div>'+
            '<div class="calendar-nextyear">&#xe664;</div>'+
        '</div>'+
        '<div class="time-set">年</div>'+
            '<div class="time-set">'+
	...
	function chushihua(){
		var tY = $(".calendar-year").html();
		var tM = $(".calendar-month").html();
        var tD = "1";
        
        var xlist = ["1-1","5-1","10-1"];

		var today = tY+"-"+tM+"-"+tD;
		var TodayTemp = new Date();
		var dateae = TodayTemp.getFullYear()+"-"+(TodayTemp.getMonth()+1)+"-"+TodayTemp.getDate();
		var datee =  $("#datee").val();
		if(typeof(datee) != 'undefined'&& datee != null && datee != '' && datee.indexOf('-')>=0){
		    var dateestr = datee.split("-");
			var chamonth = (parseInt(dateestr[0])*12 +parseInt(dateestr[1])) - (parseInt(tY)*12+parseInt(tM));
			if(datee!=""){
				showMonth(numes,chamonth);
				for(var i =0;i<$(".calendar-day").length;i++){
					if($(".calendar-day").eq(i).attr("abbr")==datee){
						$(".calendar-day").eq(i).addClass("calendar-selected");
					}
				}
				for(var i =0;i<$(".calendar-day").length;i++){
					if($(".calendar-day").eq(i).attr("abbr")==dateae){
						$(".calendar-day").eq(i).removeAttr("style");
						$(".calendar-day").eq(i).removeClass("calendar-selected");
					}
                }
	...
	function getdateList(){
		...
		if(typeof(airports)!='undefined'&&airports!=null&&airports!=''){
			...
			if(typeof(searchJson.dpt_AirPt_Cd)!='undefined'&&searchJson.dpt_AirPt_Cd!=""){
				searchJson.dpt_AirPt_Cd = airports[searchJson.dpt_AirPt_Cd].iata;
			}else{
//				return;
			}
			if(typeof(searchJson.pas_stp)!='undefined'&&searchJson.pas_stp!=""){
				searchJson.pas_stp = airports[searchJson.pas_stp].iata;
			}else{
//				return;
			}
			if(typeof(searchJson.arrv_Airpt_Cd)!='undefined'&&searchJson.arrv_Airpt_Cd!=""){
				searchJson.arrv_Airpt_Cd = airports[searchJson.arrv_Airpt_Cd].iata;
			}else{
//				return;
			}
			searchJson.isIncludeExceptionData = $('#isIncludeExceptionData').val();
			var year = $(".calendar-year").html();
			var month = $(".calendar-month").html();
			searchJson.day = year+"-"+month+"-01";
			$.ajax({
	    		url:'getDateList',
	            dataType : 'json',
	            data:searchJson,
	            success : function(datas) {
	                if(datas){
	            		$('.have-colorr').removeClass('have-colorr');
	                	 for(var i=0;i<datas.success.dateList.length;i++){
	                         for(var j=0;j< $(".time-box").find("td").length;j++){
	                             if($(".calendar-day").eq(j).attr("abbr")==datas.success.dateList[i]){
	                            	...
	                            	if($('.calendar-day').eq(j).attr('abbr')==day){
	                            		$('.calendar-day').eq(j).addClass('calendar-selected');
	                            	};
	                             };
	                         };
	                     };

                        for(var i=0;i<datas.success.fltDyDetialList.length;i++){
                            for(var j=0;j< $(".time-box").find("td").length;j++){
                                var a = new Date($(".calendar-day").eq(j).attr("abbr"));
                                var b = new Date(datas.success.fltDyDetialList[i].flyDate);
                                var as = String(a.getFullYear()) + a.getMonth() + a.getDate();
                                var bs = String(b.getFullYear()) + b.getMonth() + b.getDate();
                                if(as == bs){
                                    $(".calendar-day").eq(j).attr('dts', datas.success.fltDyDetialList[i].content);
                                    $(".calendar-day").eq(j).attr('status', datas.success.fltDyDetialList[i].status);

                                    switch (Number(datas.success.fltDyDetialList[i].status)){
                                        case 0:
                                            break;
                                        case 1:
                                            $(".calendar-day").eq(j).addClass('status-be');
                                            break;
                                        case 2:
                                            $(".calendar-day").eq(j).addClass('status-af');
                                            break;
                                        case 3:
                                            $(".calendar-day").eq(j).addClass('status-be status-af');
                                            break;
                                    }
                                };
                            };
	...
	function showMonth(target, delta) {
		var opts = $.data(target, 'fullCalendar').options;
		var yerr = parseInt(delta/12);
		opts.year = opts.year + yerr;
		delta = delta%12;
		opts.month += delta;
		if (opts.month > 12) {
			opts.year++;
	...
	// 这是日历头部
	 $(target).find('.calendar-year').html(opts.year);
     $(target).find('.calendar-month').html(opts.month);
			...
			for (var j = 0; j < week.length; j++) {
				...
				if (opts.month == day[1]) {
					...
					if (info.color) {
                        ...
                        dayHtml = '<span>' + day[2];
                        if(opts.festiveDay) {
                            for(var item of festList){
                                if(info.solarFestival.replace(/(^\s*)|(\s*$)/g, "").indexOf(item)>-1){
                                    dayHtml = '<span style="color:red;" title='+ item +' >' + day[2];
                                }
                            }
                        }
					}
